...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F)
	{
		static bool incrementando = true; // Variable para controlar la dirección

		if (incrementando)
		{
			if (theWindow->articulacion1 > -90)
			{
				theWindow->articulacion1 -= 10.0;
			}
			else
			{
				incrementando = false; // Cambia de dirección al llegar a 90
			}
		}
		...
		if (!incrementando)
		{
			...
			{
				incrementando = true; // Cambia de dirección al llegar a 0
			}
		}
	...
	if (key == GLFW_KEY_L)
	{
		static bool incrementando6 = true;
	...
	if (key == GLFW_KEY_Z)
	{
		static bool incrementando7 = true;
